...
통합 개발을 더 빠르게—OAuth 플로우를 직접 구현할 필요가 없습니다. 새로운 HighLevel PHP 및 Python SDK를 스택에 추가하면 모든 토큰 작업이 자동으로 처리되고, 타입 안전성을 보장하는 헬퍼로 모든 공개 API에 접근할 수 있습니다.

---

**목차**

- [HighLevel PHP 및 Python SDK란?](#highlevel-php-및-python-sdk란)
- [HighLevel SDK의 주요 장점](#highlevel-sdk의-주요-장점)
...
## HighLevel PHP 및 Python SDK란?

HighLevel의 공식 지원 라이브러리인 gohighlevel/api-client (PHP)와 gohighlevel-api-client (Python)는 전체 v2 Public API를 감쌉니다. 각 SDK는 OAuth 2.0 토큰 교환, 갱신, 제거 정리, 그리고 마켓플레이스 대량 설치까지 자동화하여—인증 작업 대신 기능 개발에 집중할 수 있게 해줍니다.
...
두 클라이언트 모두 HighLevel이 새 엔드포인트나 필드를 릴리스할 때마다 OpenAPI 스펙에서 자동으로 다시 빌드되므로, 간단한 composer update나 pip install --upgrade로 업데이트할 수 있습니다.
...
**Q: SDK가 HighLevel의 v1 엔드포인트와 호환되나요?**
