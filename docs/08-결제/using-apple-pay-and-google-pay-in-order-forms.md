...
이 기능은 버전 2 퍼널에서만 작동합니다. V1 퍼널을 V2로 업그레이드하는 방법은 [이 문서](why-and-how-to-upgrade-a-version-1-funnel-to-version-2-.md)를 참조하세요.
...
Stripe은 계정 설정과 기기/브라우저 호환성에 따라 Apple Pay와 Google Pay를 포함한 지원되는 결제 방법을 자동으로 활성화할 수 있습니다. Google Pay를 명시적으로 활성화하지 않았더라도 브라우저나 기기에서 Google Pay/Apple Pay를 지원하거나 계정에 Stripe의 기본 지갑 설정이 활성화되어 있으면 HighLevel 주문 폼에 나타날 수 있습니다.
