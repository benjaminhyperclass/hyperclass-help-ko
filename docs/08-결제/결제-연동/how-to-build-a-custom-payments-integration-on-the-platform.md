...
이 가이드는 HighLevel의 모든 결제 사용 사례(체크아웃, 구독, 저장된 결제 방법 등)에 연결되는 커스텀 결제 제공업체를 구축하는 방법을 안내합니다. HighLevel의 Marketplace와 Payments API를 사용하여 자체 게이트웨이나 서드파티 프로세서를 연결하고 모든 HighLevel 로케이션(하위 계정)에서 사용할 수 있게 만들 수 있습니다.
...
- [11. 7단계 – HighLevel로부터의 웹훅](#11-7단계--highlevel로부터의-웹훅)
- [12. 테스트 및 라이브 체크리스트](#12-테스트-및-라이브-체크리스트)
- [13. 자주 묻는 질문](#13-자주-묻는-질문)

## 1. 개요

HighLevel의 커스텀 결제 제공업체 프레임워크를 사용하면 모든 결제 게이트웨이를 플랫폼과 연동하고 HighLevel이 결제를 처리하는 모든 곳에서 사용할 수 있습니다.
...
- 수동 구독 및 구독 일정 지원
- 웹훅을 통해 HighLevel에 업데이트 동기화

## 2. 주요 개념

**Marketplace 앱**
HighLevel Marketplace에서 연동의 컨테이너입니다. OAuth, 스코프, 카테고리, 커스텀 페이지를 정의합니다.

**커스텀 결제 제공업체**
HighLevel에 앱이 결제 제공업체임을 알리고 지원하는 결제 유형(일회성, 정기, 오프 세션)을 설정하는 결제 구성입니다.

**queryUrl**
HighLevel이 모든 서버 측 결제 작업을 위해 호출하는 백엔드 엔드포인트(귀하의 서버)입니다:
- 결제 검증 (type: "verify")
...
**paymentsUrl**
HighLevel이 iframe 내부에서 로드하여 고객이 결제 제공업체를 통해 결제할 수 있게 하는 공개 URL입니다.

**커스텀 페이지**
HighLevel이 Marketplace UI의 iframe 내부에서 로드하는 공개 URL입니다. 연동 구성(API 키, 머천트 ID, 테스트/라이브 모드 등)에 사용됩니다.
...
- Marketplace 대시보드에 액세스할 수 있는 HighLevel 계정
...
앱이 로케이션에 설치될 때 OAuth 플로우를 완료하는 데 사용됩니다. HighLevel은 code 쿼리 매개변수와 함께 사용자를 여기로 리다이렉트합니다.
...
HighLevel은 OAuth를 위한 클라이언트 키를 제공합니다.
- 백엔드에만 저장하세요.
- OAuth 코드를 액세스 토큰으로 교환할 때 사용하세요.

#### 5.2.4 웹훅 URL(앱 설치/제거)

다음 상황에서 HighLevel이 호출할 웹훅 URL을 구성합니다:
- 로케이션에 앱이 설치될 때
- 로케이션에서 앱이 제거될 때
...
HighLevel이 SSO 키를 제공합니다.
- 백엔드에 안전하게 저장하세요.
- iframe 내부의 커스텀 페이지로 전달된 인증 토큰을 해독하여 HighLevel 사용자와 로케이션을 식별하는 데 사용하세요.

### 5.3 Payment Provider 구성

설정(Settings) 탭이 완료되면 Payment Provider 섹션을 구성합니다. 이것이 앱을 HighLevel의 "결제 앱"으로 만드는 것입니다.
...
1. HighLevel이 OAuth 코드와 함께 리다이렉트 URL을 엽니다
2. 백엔드에서 코드를 액세스 토큰으로 교환합니다(OAuth Access Token API)
3. HighLevel이 구성을 위해 커스텀 페이지를 로드합니다
4. HighLevel은 공개 제공업체 구성을 생성하는 API 호출을 기대합니다. 이는:
   - HighLevel Payments에서 제공업체를 등록합니다
...
**필드 참조**
- name (string) – HighLevel 전체에 표시되는 연동 이름(Marketplace, Integrations 등)
- description (string) – 결제(Payments) > 연동(Integrations)에 표시되는 짧은 설명/태그라인
- imageUrl (string, URL) – 결제 제공업체 로고의 공개 URL
- locationId (string) – 앱이 설치된 HighLevel 하위 계정 ID
- queryUrl (string, URL) – HighLevel이 결제 관련 작업을 위해 호출하는 백엔드 엔드포인트: verify, refund, list/charge payment methods, create subscriptions 등
- paymentsUrl (string, URL) – HighLevel이 고객 대면 결제를 위해 iframe 내부에서 로드하는 공개 체크아웃 페이지 URL

## 7. 3단계 – 테스트 및 라이브 구성 연결

설치 후 사용자는 테스트 및 라이브 자격 증명을 구성해야 합니다.

HighLevel은 Connect Config API를 통한 테스트 및 라이브 구성 업데이트를 기대합니다:

**주요 필드:**
- apiKey – HighLevel에서 queryUrl로의 백엔드 호출에 사용되는 비밀 키
...
HighLevel이 준비 이벤트를 받은 후 결제 데이터 이벤트를 보냅니다:
...
결제를 처리한 후 다음 이벤트 중 하나를 HighLevel로 다시 보내야 합니다.
...
성공을 보고하면 HighLevel은 queryUrl을 통해 백엔드 검증을 수행합니다:
...
**예상 응답**
- `{ "success": true }` - HighLevel에서 주문 및 거래를 성공으로 표시합니다
...
이 플래그를 보면 HighLevel은 설정 시작 이벤트를 보냅니다:
...
HighLevel은 저장된 방법을 가져오기 위해 queryUrl을 호출합니다:
...
저장된 방법을 사용한 오프 세션 청구나 인보이스의 경우 HighLevel이 queryUrl을 호출합니다:
...
Update Capabilities API를 사용하여 제공업체가 구독 일정을 지원한다고 HighLevel에 알립니다.

#### 9.4.2 런타임 플로우: type: "create_subscription"

수동 구독이 생성되면 HighLevel이 queryUrl에 POST를 보냅니다:
...
HighLevel이 제공업체에게 기존 구독을 취소하도록 요청할 때 사용합니다:
