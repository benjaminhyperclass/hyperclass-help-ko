...
- #### [3. Hyperclass에서는 대화를 볼 수 있는데 Eliza로는 들어오지 않아요.](#3-hyperclass에서는-대화를-볼-수-있는데-eliza로는-들어오지-않아요)
...
- #### [7. Hyperclass 하위 계정 > 설정에서 Eliza Service 탭이 보이지 않아요.](#7-hyperclass-하위-계정-설정에서-eliza-service-탭이-보이지-않아요)
...
Eliza 에이전트 플랫폼 서비스는 [여기서](https://mp.hyperclass.ai/eliza-agent-platform) 구매하실 수 있습니다.
...
- 따라서 Eliza에서 user.name에 유효한 값을 주려면, 연락처가 Hyperclass 쪽에서 누군가에게 배정되어 있어야 Eliza(와 워크플로우)가 이를 선택할 수 있습니다.
...
### 3. Hyperclass에서는 대화를 볼 수 있는데 Eliza로는 들어오지 않아요.
...
표시된 목록에서 Hyperclass 팀 멤버를 선택하세요.

Hyperclass 팀 섹션에 이미 포함된 번호만 전환할 수 있습니다. 이 통화가 전환될 팀 멤버를 선택하고 "Call & Disconnect"를 클릭하세요.

새 멤버를 추가하고 싶다면, Hyperclass에 로그인하여 해당 전화번호로 새 팀 멤버를 생성하세요.
...
### 7. Hyperclass 하위 계정 > 설정에서 Eliza Service 탭이 보이지 않아요.
