...
폴더는 콘텐츠를 정리하는 데 도움이 되고, 워크플로우는 작업을 자동화할 수 있게 해줍니다. 검증된 워크플로우를 만들기 위해 수많은 시간을 투자했는데, 승인되지 않은 변경이 발생하는 것은 원하지 않을 것입니다.
...
**워크플로우 폴더**에 마우스를 올리거나 생성하기 **>** "**Actions(작업)**" 드롭다운 버튼 클릭 **>** **Manage Permissions(권한 관리)** **>** 이 폴더와 워크플로우에 접근할 수 있는 **사용자 유형 및 역할** 선택

참고사항:

폴더 권한에 관계없이 모든 사용자는 연락처 상세 페이지(Contact Details Page) 내에서 연락처가 속한 워크플로우 목록을 여전히 볼 수 있습니다. 또한 사용자는 워크플로우에 연락처를 **추가/제거**할 수도 있습니다.
