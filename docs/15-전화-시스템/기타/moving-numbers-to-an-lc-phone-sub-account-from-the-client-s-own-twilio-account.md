...
이 가이드는 에이전시나 클라이언트가 미국 지역번호와 무료 통화번호를 Twilio에서 LC Phone 하위 계정(Hyperclass 내)으로 이전하는 단계별 방법을 설명합니다. 이를 통해 플랫폼 내에서 SMS와 음성 기능을 원활하게 사용할 수 있습니다.

**중요**: **Twilio에서 LC로** 전화번호를 이전하는 과정에서는 고객이 Hyperclass 지원팀과 Twilio 지원팀 양쪽 모두에 연락해야 성공적으로 마이그레이션할 수 있습니다.
...
- 전화번호가 마이그레이션될 Hyperclass 하위 계정의 로케이션 ID
- Hyperclass 지원팀에 요청하여 수신할 하위 계정 SID(해당 로케이션의 LC Phone Twilio 하위 계정 SID)를 받기 위한 지원 요청 제출
- 국제 번호인 경우, 필요한 규제 번들과 주소가 Twilio에서 이미 승인되어 있어야 함

## 미국 전화번호를 LC로 이전하는 주요 장점

- Hyperclass 생태계 내에서 모든 커뮤니케이션을 중앙화
...
LC Phone은 Twilio ISV(독립 소프트웨어 공급업체) 연결을 통해 작동하는 Hyperclass의 내장 전화 시스템입니다. 클라이언트의 Twilio 전화번호를 LC Phone 하위 계정으로 이전하면, 별도의 Twilio 연동 없이도 Hyperclass 내에서 전화번호가 완전히 관리됩니다. 이를 통해 사용자에게 더 안정적이고 기능이 풍부한 경험을 제공합니다. 자세한 내용은 [LC - Phone 시스템이란?](what-is-lc-phone-system-.md)을 참고하세요.

## 클라이언트의 Twilio에서 LC Phone으로 전화번호 이전

이 섹션에서는 클라이언트의 Twilio 계정에서 Hyperclass 내 LC Phone 하위 계정으로 전화번호를 마이그레이션하는 단계별 가이드를 제공합니다. 다음 지침을 주의 깊게 따라 마이그레이션을 성공적으로 완료하여 전화번호가 Hyperclass 플랫폼에 완전히 통합되어 원활한 커뮤니케이션과 관리가 가능하도록 하세요.

### 1단계: 수신할 하위 계정 SID 확인

- 전화번호를 이전할 Hyperclass 하위 계정(로케이션)으로 이동하고, Hyperclass 지원팀에 티켓을 생성하기 위해 로케이션 ID를 기록해 둡니다.
...
- Hyperclass에 지원 티켓을 제출하고 이 로케이션(하위 계정) ID에 대한 수신할 하위 계정 SID를 요청합니다.
...
Hyperclass 지원팀으로부터 수신할 하위 계정 SID를 받으면, **Twilio 지원팀에 연락**하여 **마이그레이션 프로세스를 시작**해야 합니다. **Twilio에 지원 티켓을 제출**하려면 다음 단계를 따르세요:
...
**수신할 하위 계정 SID** - Hyperclass 지원팀에 연락하여 ISV 로케이션의 Twilio 하위 계정 SID를 받아주세요
...
**중요**: 티켓을 제출하면 Twilio에서 요청을 검토하고 제공된 이메일 주소로 응답합니다. 언제든지 질문이 있거나 도움이 필요하면 Hyperclass 지원팀에 연락하세요. 프로세스 전반에 걸쳐 도움을 드리겠습니다.
...
전화번호는 클라이언트의 개인 Twilio 계정이 아닌 Hyperclass(LC Phone)을 통해 청구됩니다.
