...
**갱신** 2026-09-09 (포인터 라운드: 로더 v4.10.0 배포, 슬롯 마지막 교체 대기)

## 배포 좌표

| 항목 | 값 |
|---|---|
| 사전 버전 / REV | **v4.2.5** / `81a235c06fd7a8c65ba720b3642fe72a5a4af817` (strings 103,042 · _text 9,094) — **v4.10 부터 사전 REV 는 `data/hc-ko-app-rev.json`(포인터)이 정한다.** 현재 포인터 rev = 81a235c (by manual-init) |
...
| **대기** (v4.10 슬롯 교체 후) | 4.10.0 기대 | `81a235c…`(포인터) · `revSource` pointer-cache(2번째 로드부터) | 1 | 9,094 → 4.2.6 전진 후 9,105 | 목록 배너 "관리형 에이전트는"(공백 없음) | Claude Chat |

※ 종전에 적었던 "4.9.1 확인" 행은 실제 라이브 검증이 아니라 raw 재현값이었으므로 삭제했다.

콘솔 판정 스크립트: `scripts/verify-deploy.js` (EXPECT 4.10.0 / 81a235c / 9094 · revSource 표시).
...
| 9 | `_text` 11건 추가 → 사전 4.2.6 | 목록 수신. **포인터 방식의 첫 무슬롯 배포**로 진행(로더·슬롯 변경 없음) |
