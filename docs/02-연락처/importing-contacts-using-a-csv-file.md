...
**중요**: Hyperclass에서는 CSV(.csv) 파일 형식만 지원합니다. Excel이나 Google 스프레드시트 파일은 업로드할 수 없습니다. CSV 파일 형식에 대한 자세한 내용은 [연락처 및 기회 관리 가져오기를 위한 CSV 파일 형식](https://hyperclass.gitbook.io/hyperclass-docs) 가이드를 참고하세요.
