...
이 매칭 순서는 정확한 업데이트를 보장하고 중복을 방지합니다. 중복 제거 설정을 변경하거나 [Allow Duplicate Contacts (Contact Deduplication Preferences)](allow-duplicate-contact-explained.md)에 대해 자세히 알아볼 수 있습니다.
...
**참고**: CSV 파일에 Hyperclass에 아직 존재하지 않는 필드의 데이터가 포함된 경우, 가져오기를 완료하기 전에 해당 필드를 [Custom Fields(커스텀 필드)]([how-to-use-custom-fields](how-to-use-custom-fields.md))로 생성해야 합니다. Hyperclass에서는 CSV 컬럼을 표준 필드와 커스텀 필드 모두에 매핑할 수 있지만, 해당 필드가 계정에 이미 생성되어 있어야 합니다.
...
- [중복 연락처 허용 (연락처 중복 제거 환경설정)](allow-duplicate-contact-explained.md)
