...
* [CSV 파일 준비의 주요 장점](csv-file-format-for-importing-objects.md#csv-파일-준비의-주요-장점)
* [CSV 파일 형식이란?](csv-file-format-for-importing-objects.md#csv-파일-형식이란)
* [파일 구조화하기](csv-file-format-for-importing-objects.md#파일-구조화하기)
  * [1. CSV 형식 샘플](csv-file-format-for-importing-objects.md#1-csv-형식-샘플)
  * [2. 필수 필드](csv-file-format-for-importing-objects.md#2-필수-필드)
  * [3. 필드 형식](csv-file-format-for-importing-objects.md#3-필드-형식)
  * [4. 지원되는 파일 유형 및 크기](csv-file-format-for-importing-objects.md#4-지원되는-파일-유형-및-크기)
  * [5. 가져오기 시 필수/선택 필드](csv-file-format-for-importing-objects.md#5-가져오기-시-필수선택-필드)
  * [6. 필드 유형별 형식 가이드라인](csv-file-format-for-importing-objects.md#6-필드-유형별-형식-가이드라인)
* [CSV 가져오기에서 지원되는 국가 목록](csv-file-format-for-importing-objects.md#csv-가져오기에서-지원되는-국가-목록)
* [지원되는 시간대](csv-file-format-for-importing-objects.md#지원되는-시간대)
* [자주 묻는 질문](csv-file-format-for-importing-objects.md#자주-묻는-질문)
...
커스텀 필드 생성에 대한 도움말은 [Merge Fields & Custom Variables 개요](https://github.com/benjaminhyperclass/hyperclass-help-ko/blob/main/docs/40-%EB%B3%91%ED%95%A9%ED%95%84%EB%93%9C-%EB%B3%80%EC%88%98/Merge-Fields-&-Custom-Variables/overview-of-merge-fields-custom-variables.md)를 참조하세요.
...
**Q: 전화번호나 이메일이 여러 개인 연락처를 가져올 수 있나요?** 네! 추가 전화번호, 추가 이메일 주소와 같은 별도 열을 사용하고 항목을 쉼표로 구분하세요.

**Q: Hyperclass에 없는 필드를 CSV에 포함하면 어떻게 되나요?** 가져오기 과정에서 기존 표준 또는 커스텀 필드에 매핑하지 않는 한 해당 필드는 무시됩니다.

**Q: Hyperclass가 중복을 자동으로 감지하나요?** 네, 중복 감지 설정에 따라 연락처 ID, 전화번호 또는 이메일로 일치시켜 중복을 병합하거나 건너뛸 수 있습니다.

**Q: CSV 가져오기로 연락처를 업데이트할 수 있나요?** 물론입니다. 중복 일치에 사용되는 필드(연락처 ID, 이메일 또는 전화번호 등)를 포함했는지 확인하세요.

**Q: 빈 행과 열을 정리해야 하나요?** 네. 예상치 못한 가져오기 동작을 방지하려면 불필요한 데이터를 모두 제거하는 것이 좋습니다.

***

_원문 최종 수정: Wed, 25 Feb, 2026 at 6:14 AM_ _Hyperclass 사용 가이드 — hyperclass.ai_
