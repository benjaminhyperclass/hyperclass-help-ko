...
하이퍼클래스(Hyperclass) 계정에 누가 접근할 수 있는지 관리하는 것은 보안, 효율성, 책임 추적을 위해 매우 중요합니다. 이 문서에서는 에이전시 레벨(모든 고객 계정에 걸쳐)과 하위 계정/팀 레벨(특정 고객 계정 내)에서 사용자 접근 권한을 설정하고 제어하는 방법을 설명합니다. 또한 사용자 속성과 권한을 어떻게 구성할 수 있는지도 다룹니다.
