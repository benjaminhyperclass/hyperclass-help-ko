...
[Mailgun에서 LeadConnector로 발신 도메인 이전하기](how-to-set-up-a-dedicated-sending-domain-lc-email-.md) 가이드도 확인하세요.
...
모든 DNS 레코드를 추가하고 확인한 후에는 [전용 발신 도메인 SSL 인증서(LC - Email)](ssl-certificates-for-dedicated-lc-email-domains.md)가 제대로 설정되었는지 확인하세요.

그런 다음 테스트 이메일을 보내서 모든 것이 제대로 작동하는지 확인할 수 있습니다! [대화에서 테스트 이메일 보내는 방법](how-to-send-a-test-email-in-the-conversation.md)을 확인하세요.
