...
# WIX를 도메인 공급업체로 사용할 때 LC 이메일/Mailgun 답장이 작동하지 않는 문제

WIX에서 도메인을 어디서 구매하셨나요?

네임서버가 WIX에 의해 관리되지 않도록 설정하기 위해 다음 가이드를 따라야 합니다.
...
그리고 "I will use GoDaddy Nameservers"를 선택하세요:
...
이 가이드로 이동하여 DNS 레코드에 추가할 레코드를 복사하세요:
...
기존 CNAME 레코드를 편집하고 여기에 wwwXX.wixdns.net 레코드를 붙여넣기하세요:
...
yourdomain.com (A 레코드)를 입력하고 검색해서 작동하는지 확인하세요:
...
[www.yourdomain.com](http://www.yourdomain.com/) (CNAME 레코드)를 입력하고 검색해서 작동하는지 확인하세요:
...
yourdomain.com (NS 레코드)를 입력하고 검색해서 작동하는지 확인하세요:
...
yourdomain.com 웹사이트로 이동하여 웹사이트가 정상 작동하는지 확인하세요.

이제 다음 가이드를 따라 도메인 공급업체의 DNS 레코드에서 Mailgun을 설정할 수 있습니다!
...
---
*원문 최종 수정: Thu, 27 Jul, 2023 at 12:34 PM*
