...
1. 하위 계정 Settings(설정)을 클릭하고 My Profile(내 프로필) 옵션을 선택한 다음, Calendar Settings(캘린더 설정) 아래의 **+ Add New(새로 추가)** 버튼을 클릭하세요. [Hyperclass에 구글 캘린더 연동하기](integrating-google-with-highlevel-calendars.md) 가이드를 참조하여 구글 계정을 성공적으로 연결할 수도 있습니다.
